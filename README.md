...
支持两种接入方式：**uvx**（推荐，无需手动克隆）和**本地路径**。

### 方式一：uvx（推荐）

需要先安装 [uv](https://docs.astral.sh/uv/getting-started/installation/)，然后在配置文件中直接引用 GitHub 仓库，无需手动克隆和安装：
...
Cursor、Claude Code 等客户端配置格式相同，只是配置文件路径不同（见下方）。
