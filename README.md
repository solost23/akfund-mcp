...
5. 输出新增资金推荐：调用 get_sector_quotes() 扫描全市场板块，找近1周涨幅前5的板块（判断资金流向），
   再用 search_fund() + get_multi_fund_rank() + get_multi_fund_metrics() 筛选：
   近3月+近1月同类排名均进前25%，且当前位置为中位或低位（高位不推荐，避免高位接盘），
   结合消息面给出 1-2 只买入候选及建议金额；
   未提供新增资金金额时默认按 1000元 执行，无符合条件候选时输出"建议持币等待回调"
```
...
| 工具 | 参数 | 说明 |
|---|---|---|
| `get_daily_brief` | `sectors=[...]`, `keywords=[...]`, `news_pages=8`, `codes=[...]` | 一次并发调用获取全部每日行情数据（市场+板块+所有新闻+基金盘中估值） |
