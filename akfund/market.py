...
def get_market_quotes(markets: dict[str, str] | None = None) -> dict[str, Quote | dict]:
    """
    Fetch real-time quotes for market indices, FX, and commodities (single batch request).
    一次批量请求抓取市场指数、汇率、大宗商品实时行情。
    ...
    secid_to_name = {secid: name for name, secid in markets.items()}
    rows = request_batch(list(markets.values()), fields="f12,f14,f2,f3,f4")

    results: dict[str, Quote | dict] = {}
    returned: set[str] = set()
    for row in rows:
        f12 = row.get("f12", "")
        secid = next((s for s in markets.values() if s.endswith(f12)), None)
        if not secid:
            continue
